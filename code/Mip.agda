...
... | inj₁ ([] , refl , refl) = 
  ...
  in intrp (D ⇒ D') (⇒L h g') (⇒R (⇒L {[]} g h')) -- principal but ⇒
... | inj₁ (B ∷ Ω , refl , refl) = 
...
mip Γ (A ∷ Δ) ._ (⇒L {._} {Δ₁} {Λ₁} {A₀} {B₀} f f₁) eq | inj₂ (E , ._ , eq1 , refl) | inj₁ (Ω' , refl , refl) | inj₂ (Ω'' , refl , eq4) | inj₁ (Ω''' , refl , refl) = 
  ...
  in intrp (D' ⊗ D) (⊗L (⇒L {_ ++ _ ∷ []} g g')) (⊗R h' h) -- principal ⊗
...
--   let intrp D g' h' = mip (Γ₁ ++ B₀ ∷ Ω'') (A ∷ Δ) Λ f₁ refl -- right in f₁
--   in intrp D (⇒L f g') h'
...
--   let intrp D g' h' = mip Γ (E ∷ Ω''' ++ B₀ ∷ Ω') Λ f₁ refl -- middle in f₁
...
--   let intrp D g' h' = mip (E ∷ Ω'') (A ∷ Δ) Ω' f refl -- in f
--   in intrp D (⇒L g' f₁) h'
-- mip Γ (A ∷ Δ) Λ (⇒L {Γ₁} {Δ₁} {Λ₁} {A₀} {B₀} f f₁) refl | inj₂ (E , Ω , refl , refl) =
--   let intrp D g' h' = mip Γ (A ∷ Δ) (E ∷ Ω ++ B₀ ∷ Λ₁) f₁ refl -- left in f₁
