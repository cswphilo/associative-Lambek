...
The applicability of the method usually relies on the existence of a cut-free sequent calculus for the logic under investigation while there are some recent work generalize the idea to calculi with primitive cut rules, including \cite{KOWALSKI2016,Hetzl2024,Saurin2025,Corbard2025}.
The triple $(D,g,h)$ consisting of a formula $D$ and derivations $g$ and $h$ outputted by the interpolation procedure is called an \emph{interpolation triple}.
...
We also ask what is invariant under the interpolation procedure when the input derivation is changed by proof equivalence.
We call this the \emph{well-definedness} of Maehara's interpolation procedure.
For \NL, Veltri and Wan showed that the invariant is the interpolant formula itself \cite{VW2025Tableaux}.
Equivalent derivations produce the same interpolant formula, up to the expected equivalence of the accompanying derivations.
...
We therefore propose a \emph{zig-zag} equivalence of interpolation triples which implies the well-definedness of the interpolation procedure in \L.
...
Thus the well-definedness result identifies the level at which Maehara interpolation remains canonical in the associative setting.
...
To increase our confidence in the correctness of the theorem, we formalized all cases of the result for \L\ in Agda 2.8.0.
The formalization covers the calculus, cut admissibility, Maehara interpolation, proof-relevant interpolation, and well-definedness result discussed above; the code is available in the \href{https://anonymous.4open.science/r/code-associative-Lambek/}{repository} on Anonymous GitHub.
...
$$A, B ::= X \ | \ A \Rarr B \ | \ B \Larr A \ | \ A \ot B$$
