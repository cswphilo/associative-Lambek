...
% LIPIcs/CSL version adapted from the IJCAr manuscript.
% See lipics-v2021-authors-guidelines.pdf for style details.
\pdfoutput=1
\hideLIPIcs
...
\newcommand{\unitl}{\I\mathsf{L}}
\newcommand{\unitr}{\I\mathsf{R}}
...
\newcommand{\agdaurl}{https://anonymous.4open.science/r/code-associative-Lambek}
\newcommand{\agdahen}{\begingroup\setlength{\fboxsep}{0.2pt}\setlength{\fboxrule}{0.3pt}\raisebox{-1pt}{\includegraphics[height=2ex]{agdahen.png}}\endgroup}
\newcommand{\agdaref}[1]{\href{\agdaurl/#1}{\agdahen}}
...
\author{Niccol{\`o} Veltri}{Tallinn University of Technology, Tallinn, Estonia}{niccolo@cs.ioc.ee}{https://orcid.org/0000-0002-7230-3436}{}
% \author{Cheng-Syuan Wan}{Tallinn University of Technology, Tallinn, Estonia}{cswan@cs.ioc.ee}{https://orcid.org/0000-0003-2053-1688}{}
% \authorrunning{N. Veltri and C.-S. Wan}
% \Copyright{Niccol{\`o} Veltri and Cheng-Syuan Wan}

\ccsdesc[500]{Theory of computation~Proof theory}
\keywords{Lambek calculus, Maehara's method, proof-relevant interpolation, equivalence of derivations}
\category{}
\relatedversion{}
\supplementdetails[linktext={Anonymous code repository}, subcategory={Agda formalization}]{Software}{https://anonymous.4open.science/r/code-associative-Lambek/}
\funding{This work was supported by the Estonian Research Council grant PSG749.}

% Editor-only macros (set to CSL placeholders for a submission draft).
\EventEditors{}
\EventNoEds{0}
\EventLongTitle{Computer Science Logic (CSL 2027)}
...
This work investigates technical aspects of the Maehara interpolation property and Maehara's method, typically employed as an effective proof-theoretic method for establishing Craig interpolation in substructural logics.
  ...
  We show that, in the sequent calculus formulation of Lambek calculus, the Maehara interpolation procedure is a right inverse of the admissible cut rule, a property recently named ``proof-relevant'' or ``Maehara-Čubrić'' interpolation.
...
But proof theorists are not only interested in the way in which proofs can be constructed, they also care about \emph{equality of proofs}: deductive systems are specified by a set of deductive rules \emph{together} with a set of equality rules. The latter typically consist of $\beta$- and $\eta$-equivalences plus a large number of permutative conversions.
Proof theorists can produce a set of equations, which generate a congruence relation $\circeq$ on derivations, in various ways. Possibly as an axiomatization of equality in categorical models, e.g. two derivations (or terms) $f,f'$ in intuitionistic logic should be considered equal, i.e. $f \circeq f'$, iff the interpretations of $f$ and $f'$ are equal in every Cartesian closed category \cite{lambekscott1988}. Alternatively, the equality rules can be obtained by studying geometric representations of proofs, e.g. in linear logic $f \circeq f'$ iff $f$ and $f'$ have the same proof net \cite{girard:linear:87}.

In this paper, we are interested in the relationship between equivalence of derivations and interpolation properties, such as Craig interpolation and its generalizations, in substructural logics. These are logical systems allowing a fine control on the use and production of formulae/resources. Concretely, we study this relationship in the sequent calculus formulation of \emph{Lambek's syntactic calculus} (\L) \cite{lambek:mathematics:58}, which forbids the structural rules of weakening, contraction and exchange,
%The sequent calculus formulation of Lambek calculus (\L) has the sequents of the form $\GG \vd C$ where the $\GG$ is an oredered list of formulae.
and has been extensively studied for its linguistic applications \cite{moot:categorial:2012}.
...
A generalization of Craig interpolation is \emph{Maehara interpolation}, also known as strong deductive interpolation. In \L, this property states that for a derivation $f : \GG , \GD , \GL \vd C$ with a specified sublist $\GD$ of the antecedent, there exist a formula $D$ and two derivations $g : \GG, D, \GL \vdash C$ and $h : \GD \vdash D$ such that $\vars{D} \subseteq \vars{\GD} \cap \vars{\GG, \GL, C}$.
The Maehara interpolation property is usually proved by Maehara's method \cite{maehara1961}, i.e.\ by induction on the derivation $f$. We call this method \emph{Maehara's interpolation procedure}.

The applicability of the method usually relies on the existence of a cut-free sequent calculus for the logic under investigation although recent work extends the idea to calculi with primitive cut rules \cite{KOWALSKI2016,Hetzl2024,Saurin2025,Corbard2025}.
The triple $(D,g,h)$ consisting of a formula $D$ and derivations $g$ and $h$ produced by the interpolation procedure is called an \emph{interpolation triple}.
...
Traditionally, the Maehara interpolation property gives an interpolation triple $(D,g,h)$, but it does not say how the input derivation relates to this triple.
In this work, we are interested in a \emph{proof-relevant} version of Maehara interpolation, also known as \emph{Maehara-\cub\ interpolation}, that exhibits such a relationship.
In \L, proof-relevant interpolation states that, if the interpolation procedure on $f$ produces an interpolation triple $(D,g,h)$, then $\cut \ (h , g) \circeq f$.
Here $\cut \ (h,g)$ denotes the cut-free derivation produced from $h$ and $g$ by the cut-admissibility procedure.
In \cite{Saurin2024}, this is understood as the introduction of instances of cut.
The equivalence between $\cut (h , g)$ and $f$ shows that such cut-introduction is safe at the level of proof equivalence.
This line of work was initiated in the 90s by \cub~\cite{vcubric1993results,Cubric1994}, who proved proof-relevant interpolation for the simply typed $\lambda$-calculus with sum types,
...
Veltri and Wan studied proof-relevant interpolation for a semi-associative fragment of \L~\cite{VW2025}, as well as for the nonassociative Lambek calculus (\NL) \cite{VW2025Tableaux}.

In this paper, we generalize this analysis to the associative Lambek calculus.
We also ask a second question: how are interpolation triples related when the input derivations are equivalent?
We call this the \emph{well-definedness} of Maehara interpolation procedure.
Conceptually, this is about what remains stable in Maehara interpolation when the input derivations are equivalent.

For \NL, Veltri and Wan showed that two interpolation triples $(D , g, h)$ and $(D' , g', h')$ can be considered equivalent if $D = D'$, $g \circeq g'$ and $h \circeq h'$ \cite{VW2025Tableaux}. In particular, the interpolant formulae $D$ and $D'$ produced by equivalent derivations $f \circeq f'$ are the same.

In \L\ this is no longer true.
Equivalent derivations may produce different, and even logically inequivalent, interpolant formulae, e.g.\ the pair of equivalent derivations in (\ref{eq:ct:example}) has distinct and logically inequivalent interpolation formulae.
We therefore propose a \emph{zig-zag} equivalence of interpolation triples.
The equivalence of triples $(D , g, h) \sim (D' , g', h')$ is the symmetric transitive closure of the relation stating that there exists a derivation $t : D \vd D'$ such that $g \circeq \cut \ (t , g')$ and $\cut \ (h , t) \circeq h'$.
The well-definedness result thus states that if $f \circeq f'$, then $(D , g, h) \sim (D' , g', h')$, where $(D , g, h)$ and $(D' , g', h')$ are the interpolation triples of $f$ and $f'$, respectively.

Finally, we look at the \emph{product-free} Lambek calculus (\LPfree), i.e. the fragment of \L\ without multiplicative conjunction.
Pentus showed that \LPfree\ does not enjoy Maehara interpolation, but Craig interpolation can still be proved using a more complex \emph{Maehara multi-interpolation} property \cite{Pentus1997}. We conclude our paper by establishing a proof-relevant version of Pentus' multi-interpolation and studying an appropriate equivalence of multi-interpolation tuples that implies well-definedness of the multi-interpolation procedure. 

\subsection*{Formalization}
% The proof of Theorem \ref{thm:welldef:L} requires a great amount of case distinction.
...
% Keeping track of all these cases is therefore error-prone.
To increase our confidence in the correctness of the theorems, we formalized the following results for \L: the calculus, cut admissibility (Theorem \ref{thm:cut}), Maehara interpolation (Theorem \ref{thm:MIP}), proof-relevant interpolation (Theorem \ref{thm:cut:intrp:L}), and well-definedness (Theorem \ref{thm:welldef:L}). The code is available in the \href{https://anonymous.4open.science/r/code-associative-Lambek/}{repository} on Anonymous GitHub.

We postulate that, if $g \circeq g'$, then $\cut\ (f,g) \circeq \cut\ (f,g')$. We expect the proof to follow the same induction-and-case-analysis pattern as Theorem \ref{thm:welldef:L}.

% The formalization was implemented in collaboration with Anthropic Claude Opus 4.8 and OpenAI GPT 5.5.
...
Formulae of \L\ are inductively generated by the grammar (\agdaref{Fma.agda\#L23}):
\[A, B ::= X \ | \ \I \ | \ A \Rarr B \ | \ B \Larr A \ | \ A \ot B\]
where $X$ is drawn from a set $\mathsf{At}$ of atomic formulae, ${\Rarr}$ and ${\Larr}$ are implications/residuals and ${\ot}$ is fusion/multiplicative conjunction.
...
Derivations of \L\ are inductively generated by the following rules (\agdaref{SeqCalc.agda\#L11}):

\begin{equation*}\label{eq:seqcalc}
\begin{array}{c}
  \infer[\ax]{A \vd A}{}
  \quad
  \infer[\unitr]{\quad \vd \I}{}
  \quad
  \infer[\unitl]{\GG , \I , \GD \vd C}{
    \GG , \GD \vd C
  }
  \quad
...
\begin{theorem}[Cut admissibility for \L\ \cite{moot:categorial:2012} (\agdaref{Cut.agda\#L26})]\label{thm:cut}
...
\end{theorem}
\begin{proof}
  The proof proceeds by induction on the derivation of the second premise and the complexity of the cut formula.
...
\begin{theorem}[Maehara interpolation for \L\ \cite{roorda1991,moot:categorial:2012} (\agdaref{Mip.agda\#L22})]\label{thm:MIP}
...
\begin{proof}
  ...
  The definition of $\mf{mip}$ proceeds by recursion on the derivation $f$.
...
%  For example, if $\GD$ is contained in $\GD'$, i.e.\
  ...
  with $\vars{D} \subseteq \vars{\GD} \cap \vars{\GD'_0, \GD'_1,A}$.
  Then $\mf{mip}\ (\GD,f) = (D,g,h')$ where $g = \Rl \ (g' , f'')$.
  The variable condition is satisfied since $\vars{D} \subseteq \vars{\GD'_0, \GD'_1,A} \subseteq \vars{\GD'_0, \GD'_1,A\Rarr B, C}$.
%   \[
%     \begin{array}{c}
%     g = \proofbox{
...
Cases $(ii)$ $\GD$ is contained in $\GG'$, $(iii)$ $\GD$ is contained in $\GL'$ and $(iv)$ $\GD$ contains $\GD' , A \Rarr B$, are treated similarly.
...
%     \end{itemize}
  ...
  \begin{array}{c}
    g = \proofbox{
      \infer[\tl]{\GG'_0 , E \ot D  , \GD'_1,A \Rarr B , \GL' \vd C}{
        \infer[\Rl]{\GG'_0 , E , D  , \GD'_1,A \Rarr B , \GL' \vd C}{
 ...
 % The variable condition is easy to check.
  ...
  Case $(vi)$: $\GD$ splits between $\GD'$ and $\GL'$ and contains $A \Rarr B$. Then $\GD'$ has the form $\GD'_0 , \GD'_1$ and $\GL'$ has the form $\GL'_0 , \GL'_1$ so that $\GD = \GD'_1 , A \Rarr B , \GL'_0$.
...
%    Then The desired interpolant formula is $D \Rarr E$ and the desired derivations are constructed as follows:
    ...
    \end{array}
    \end{equation}
    In both cases the variable condition is easy to verify.
\end{proof}
...
In the sequent calculus, some sequents may have different derivations, but 
some of them are ``morally'' the same, they differ syntactically because of the flexibility of the inference rules.
For example, the sequent $\GG , \GD , A \Rarr B , \GL \vd A' \Rarr B'$ in \L\ has two different proofs, assuming given $f : \GD \vd A$ and $f' : A' , \GG , B , \GL \vd B'$:
\[
\begin{array}{c}
...
This semantic notion of equality can be axiomatized and gives rise to an equivalence of derivations $\circeq$ (\agdaref{SeqCalc.agda\#L57}).
...
($i$) $\eta$-equivalences, i.e.\ each instance of the rule $\ax$ on a compound formula can be expanded using left and right rules to derivations using $\ax$ on simpler formulae, e.g.\ see the $\eta$-expansion of $\ax$ on $A \Rarr B$ in (\ref{eq:eta:tr});
($ii$) permutation of left and right rules, e.g.\ see how $\tr$ permutes with $\Rl$ in (\ref{eq:tr:rl:comm});
($iii$) permutation of \emph{consecutive} two consecutive applications of left rules on different formulae and in different sequents, e.g.\ see how $\Rl$ permutes with $\Ll$ in (\ref{eq:rl:ll:assoc});
($iv$) permutation of \emph{disjoint} applications of left rules, e.g.\ see how two instances of $\Rl$ permute with each other in (\ref{eq:rl:comm}).
% \begin{figure}[t]
% \normalsize
...
% \label{fig:conv}
% \end{figure}
The admissible cut rule is proved to satisfy a number of equivalences , stating that $\cut$ commutes with left introduction rules in the first premise up to $\circeq$, e.g.
$\cut\ (\Rl \ (f' , f''), g) \circeq \Rl \ (f',\cut \ (f'',g))$ (\agdaref{CutProperties.agda\#L319}).
The analogous equivalences hold for $\Ll$ and $\tl$.
The congruence relation $\circeq$ does not include generating equations for $\beta$-equivalences, but these are visible in the cut admissibility theorem, they are the \emph{syntactic equalities} in (\ref{eq:beta}).

Since $\circeq$-related proofs are considered equal, i.e. the set of derivations of a sequent $\GG \vd C$ gets quotiented by $\circeq$,
each construction on derivations must be well-defined wrt. $\circeq$. For example, the cut rule is well-defined: if $f \circeq f'$ and $g \circeq g'$, then $\cut\ (f,g) \circeq \cut \ (f',g')$.
...
\begin{theorem}[Proof-relevant interpolation for \L\ (\agdaref{CutIntrp.agda\#L29})]\label{thm:cut:intrp:L}
  Given $f : \GG , \GD , \GL \vd C$, if $\mf{mip}\ (\GD,f) = (D,g,h)$, then $\cut \ (h , g) \circeq f$.
\end{theorem}
\begin{proof}
  By induction on the derivation $f$.
  We only discuss the case $f = \Rl \ (f' , f'')$ and its subcases $(v)$ and $(vi)$ in the proof of Theorem \ref{thm:MIP}. %in the the proofs of the interesting subcases of $f = \Rl \ (f' , f''): \GG' , \GD' , A \Rarr B , \GL' \vd C$ mentioned
...
In nonassociative Lambek calculus (\NL), two interpolation triples $(D , g , h)$ and $(D' , g' , h')$ can be considered equivalent if $D = D'$, $g \circeq g'$ and $h \circeq h'$\footnote{Here the relation $\circeq$ is the equivalence of derivations in \NL\ described in \cite{VW2025Tableaux}}. Veltri and Wan showed in \cite{VW2025Tableaux} that the Maehara interpolation procedure in \NL\ is well-defined, in the sense that if it receives in input two $\circeq$-related derivations $f$ and $f'$, it returns equivalent interpolation triples.
...
The left one entails the right one, but not the other way around.
...
\begin{definition}[Equivalence of interpolation triples (\agdaref{IntrpTriples.agda\#L21})]\label{def:equiv:intrp:triples}
  Two triples are equivalent, written $(D , g , h) \sim (D' , g' , h')$, if they satisfy the symmetric transitive closure of the following relation: 
...
The zig-zag is necessary because equivalent derivations need not produce interpolant formulae that are connected by a simple, directed chain of derivations. In particular, there are equivalent derivations whose interpolants do not imply each other, but can imply or be implied by the interpolant extracted from a third equivalent derivation.

There can be equivalent derivations $f$ and $f'$ whose interpolants
\[
((D_0 \Rarr E_0) \ot F_0) \ot ((D_1 \ot F_1) \Larr E_1)
\qquad\text{and}\qquad
(D_0 \Rarr (E_0 \ot F_0)) \ot (D_1 \ot (F_1 \Larr E_1))
\]
do not imply one another, while both are implied by
\[
(D_0 \Rarr (E_0 \ot F_0)) \ot ((D_1 \ot F_1) \Larr E_1),
\]
the interpolant of another derivation equivalent to both $f$ and $f'$.
\addtocounter{example}{-1}
\begin{example}[continued]\label{example}
We check that derivations in (\ref{eq:ct:example}) have equivalent interpolation triples.
Take $t = \Rr \ (\tl \ (\tr \ (\Rl \ (\ax , \ax),\ax))$ proving the sequent $(D \Rarr E) \ot F \vd D \Rarr (E \ot F)$.
We show that $\cut \ (t , g_1) \circeq g_0$ and $\cut \ (h_0 , t) \circeq h_1$:
\[
\begin{array}{lr}
  \cut \ (t,g_1) &
  \\
...
\]
\end{example}
\begin{theorem}[Well-definedness of $\mf{mip}$ (\agdaref{IntrpWellDef.agda\#L57})]\label{thm:welldef:L}
  Given derivations $f , f' : \GG , \GD , \GL \vd C$ with $\mf{mip}(\GD,f) = (D , g , h)$ and $\mf{mip}(\GD,f') = (D' , g' , h')$, if $f \circeq f'$ then $(D , g , h) \sim (D' , g' , h')$.
\end{theorem}
\begin{proof}
  By induction on the proof of $f \circeq f'$ (remember that $\circeq$ is an inductively defined congruence).
  We only show the case of the generating equation in (\ref{eq:rl:comm}), other cases are similar.
  We proceed in different ways depending on the location of $\GD$ inside the context $\GG , \GD_0 , A \Rarr B , \GL , \GD_1 , A' \Rarr B', \GO$, which is the antecedent in the conclusion of both derivations.
  There are 15 possible cases depending on how the interpolation context $\GD$ splits the antecedent of the endsequent, but they fall into three main situations.
...
The triples are equivalent by (\ref{eq:rl:comm}).
...
The triples are equivalent since the interpolant formulae are equal and the derivations are $\circeq$-related.


Situation $(ii)$: $\GD$ splits between $\GL$ and $\GD_1$. This implies that $\GL$ has the form $\GL_0 , \GL_1$ and $\GD_1$ has the form $\GD'_{1} , \GD''_{1}$ so that $\GD = \GL_1 , \GD'_1$. Computing $\mf{mip}$ gives two interpolation triples
\[
\begin{array}{l}
...
In all these cases the interpolation triples are computed using the recursive call on \emph{two} derivations among $f,f'$ and $f''$.
The triples are equivalent since the interpolant formulae are equal and the derivations are $\circeq$-related.
...
The main difference from the situation in the presence of products is that \LPfree\ \emph{does not} enjoy Maehara interpolation.
The reason is visible in the proof of Theorem \ref{thm:MIP}: in case $(v)$ the interpolant formula is a product $E \ot D$.
...
%              \deduce{\GG , B , \GL \vd C}{g}
  ...
  \end{array}
\end{equation*}
If the list of derivations is empty, then $\Rsl \ ([~] , g) = g$.
...
Given a list of contexts $[\GD_1,\dots,\GD_n]$, we say that it \emph{partitions} a context $\GD$ if $\Delta = \GD_1, \dots, \GD_n$.
\begin{theorem}[Maehara multi-interpolation for \LPfree\ \cite{Pentus1997}]\label{thm:MIP:LPfree}
  Given a derivation $f : \GG , \GD , \GL \vd C$, there exist a list of contexts $[\GD_1, \dots, \GD_n]$ partitioning $\GD$, formulae $D_1,\dots,D_n$, a derivation $g : \GG, D_1,\dots,D_n, \GL \vdash C$ and derivations $h_i : \GD_i \vdash D_i$ for $i \in \{1,\dots,n\}$ such that $\vars{D_1 , \dots, D_n} \subseteq \vars{\GD} \cap \vars{\GG, \GL, C}$.
\end{theorem}
\begin{proof}
  We build a function $\mf{mmip}$ producing a tuple $(n,\vec{\GD},\vec{D},g,\vec{h})$ for each derivation $f : \Gamma,\Delta,\Lambda \vdash C$ and write $\mf{mmip}\ (\GD,f) = (n,\vec{\GD},\vec{D},g,\vec{h})$. Here the list $\vec{\GD}$ has length $n$ and its $i$th element is denoted $\GD_i$, and similarly for lists $\vec{D}$ and $\vec{h}$.
  The construction proceeds similarly to the construction of $\mf{mip}$ in Theorem \ref{thm:MIP}.
...
%  $\GD$ splits between $\GG'$ and $\GD'$, then $\GG'$ has the form $\GG'_0 , \GG'_1$ and $\GD'$ has the form $\GD'_0 , \GD'_1$ so that $\GD = \GG'_1 , \GD'_0$.
  ...
  % \end{itemize}
  \begin{itemize}
  \item a list of contexts $[\GD'_{0_1}, \dots , \GD'_{0_n}]$ partitioning $\GD'_0$ ,
  \item formulae $D_1,\dots,D_n$,
  \item a derivation $g' : D_1,\dots,D_n , \GD'_1 \vd A$,
  \item derivations $h'_i : \GD'_{0_i} \vdash D_i$ for $i \in \{1,\dots,n\}$ such that
    \item $\vars{D_1 , \dots, D_n} \subseteq \vars{\GD'_0} \cap \vars{\GD'_1, A}$
  \end{itemize}
  % \begin{itemize}
  ...
  % \end{itemize}
  \begin{itemize}
  \item a list of contexts $[\GG'_{1_1}, \dots , \GG'_{1_m}]$ partitioning $\GG'_1$,
  \item formulae $E_1, \dots,E_m$,
  \item a derivation $g'' : \GG'_0 , E_1, \dots,E_m, B , \GL' \vd C$,
  \item derivations $h''_j : \GG'_{1j} \vdash E_j$ for $j\in\{1,\dots,m\}$ such that
  \item $\vars{E_1 , \dots, E_m} \subseteq \vars{\GG'_1} \cap \vars{\GG'_0 , B , \GL' , C}$
  \end{itemize}
  ...
  We return $\mf{mmip}\ (\GD,f) = (l,\vec{\GD},\vec{F},g,\vec{h})$, where $l = n+m$,  the list of contexts $\vec{\GD}$ is $\GG'_{1_1}, \dots , \GG'_{1_m}, \GD'_{0_1}, \dots , \GD'_{0_n}$ (which partitions $\GD$), the interpolant formulae $\vec{F}$ are $E_1 , \dots, E_m, D_1,\dots, D_n$, the derivation $g = \Rl \ (g' , g'')$ and derivations $\vec{h}$ are $h''_1, \dots, h''_m, h'_1, \dots, h'_n$.
  The variable condition is satisfied since $\vars{E_1,\dots,E_m,D_1,\dots,D_n} \subseteq \vars{\GG'_1,\GD'_0} \cap \vars{\GG'_0,\GD'_1,A\Rarr B,\GL',C}$.
  ...
  \end{itemize}
  \begin{itemize}
  \item a list of contexts $\GL'_{0_1}, \dots , \GL'_{0_m}$ partitioning $\GL'_0$, where $\GL'_{0_j}$ has the form $B , \GL''_{0_1}$,
  \item formulae $E_1,\dots,E_m$ for $j\in\{1,\dots,m\}$,
  \item a derivation $g'' : \GG' , E_1,\dots,E_m, \GL'_1 \vd C$,
  \item derivations $h''_j : \GL'_{0_j} \vdash E_j$ for $j \in \{1,\dots,m\}$ such that
  \item $\vars{E_1 , \dots, E_m} \subseteq \vars{\GG'_1} \cap \vars{\GG,  \GL'_1 , C}$
  \end{itemize}
  ...
  % The variable condition is easy to check.
  We return $\mf{mmip}\ (\GD,f) = (m,\vec{\GD},\vec{F},g,\vec{h})$, where the list of contexts $\vec{\GD}$ is $(\GD'_1, A \Rarr B , \GL''_{0_1}), \GL'_{0_2}, \dots \GL'_{0_m}$ (which partitions $\GD$), the interpolant formulae  $\vec{F}$ are $(D_1,\dots,D_n) \Rs E_1 , E_2 , \dots , E_m$, the derivation $g = \Rsl \ (\vec{h'} , g'')$ and derivations $\vec{h}$ are $\Rsr \ (\Rl \ (g' , h''_1)), h''_2, \dots , h''_m$.
The variable condition is easy to verify.
\end{proof}
...
Given a list $\vec{\GD}$, we write $\vec{\GD}[i..j]$ for the sublist of elements between positions $i$ and $j$.
\begin{definition}[Equivalence of multi-interpolation tuples]\label{def:equiv:intrp:triples:LPfree}
%~
%\\
  Two tuples are equivalent, written $(n,\vec{\GD}, \vec{D} , g , \vec{h}) \sim  (m,\vec{\GD'} , \vec{D'} , g' , \vec{h'})$, if they satisfy the symmetric transitive closure of the following relation:
...
%     i.e. there is an equality $\vec{\GD} = \linebreak(\GD_1,\dots,\GD_{n_1}), \dots,(\GD_{n_{m-1}+1},\dots,\GD_{n_m})$,
%   \item $\GD'_j = \GD_{n_{j-1}+1},\dots,\GD_{n_{j}}$ for $j \in \{1,\dots,m\}$ (where $n_0$ is assumed to be 0),
  \item there exist $n_0,n_1,\dots,n_m \in \mathbb{N}$ such that $0 = n_0 < n_1< \dots <n_m = n$ and $\vec{\GD}[n_{j-1}+1..n_{j}]$ partitions $\GD'_j$ for all $j \in \{ 1,\dots,m \}$,
  \item there exist derivations $t_j : D_{n_{j-1}+1},\dots,D_{n_{j}} \vd D'_j$ for each $j \in \{ 1,\dots,m \}$, 
  \item $g \circeq \cuts \ (\vec{t} , g')$ and
  \item $\cuts \ (\vec{h}[n_{j-1}+1..n_j] , t_j) \circeq h'_j$ for each $j \in \{ 1,\dots,m \}$.
  \end{enumerate}
...
%%   \end{definition}
 ...
 Let us write $\vec{t} : \vec{D} \vdash \vec{D'}$ to denote the list of derivations in condition 2 of Definition \ref{def:equiv:intrp:triples:LPfree}.
...
Computing $\mf{mmip}$ on the two sides of equation (\ref{eq:rl:ll:assoc}) returns
\[
\begin{array}{c}
     \begin{array}{c}
       \text{(for the left-hand-side)} \\
...
\]
  ...
  %     \end{array}
  %   \]
Condition 1 of Definition \ref{def:equiv:intrp:triples:LPfree} is satisfied by taking $0 < 1 < 2 < \dots < l-1 < l + n$, so that $[\GG'_k]$ partitions $\GG'_k$ for $k \in \{1,\dots,l-1\}$ and $[(\GG''_{1_l} , B'\Larr A',\GG),\GD_{0_1},\dots,\GD_{0_n}]$ partitions  $\GG''_{1_l} , B'\Larr A',\GG,\GD_{0_1},\dots,\GD_{0_n}$.
...
%   }
% \]
Condition 2 holds by constructing the derivation $t = \Lsr \ (\Rsl \ (\vec{\ax}, \Lsl \ (\vec{\ax}, \ax)))$.
%The other parts of the two partitions can be ignored since they are exactly the same.
Condition 3 is true since we calculate
...
At first glance the zig-zag relation might look quite unusual, but it is actually very natural from a category-theoretic perspective, since it characterizes the collection of interpolation triples as some kind of colimit/coend \cite{Loregian_2021}.
The sequent calculus of \L\ presents a multicategory, where derivations are multimorphisms and admissible cut is the composition of multimorphisms.
From this perspective, the statement of Maehara interpolation is reminiscent of Roman's \emph{malleable multicategory} \cite{Roman2023}.  Interpolation triples can be viewed as factorizations of a multimorphism through an intermediate object, and the zig-zag equivalence is the appropriate quotient of such factorizations. 
We plan to expand on this categorical characterization in future work.
...
We would also like to study the well-definedness of interpolation triples in more permissive systems, e.g.\ sequent calculi with cuts \cite{Saurin2025} and the calculi with complete Maehara interpolation procedures \cite{Hetzl2024}.
